    ...
    <div class="flex min-h-[calc(100vh-56px)]">
      ...
      <div class="flex-1 min-w-0">
        ...
        >
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {{ formatDate(endDate) }} &middot; Page
          ...
          <!-- ==================== KPI CARDS ==================== -->
          <div
            class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 print:grid-cols-4 print:gap-2"
          >
            <div
              class="relative overflow-hidden rounded-2xl border-2 border-emerald-200 bg-emerald-50/50 p-4 shadow-sm dark:border-emerald-800/50 dark:bg-emerald-950/20 print:rounded-lg print:border-slate-900 print:bg-white print:p-3 print:shadow-none"
            >
              <p
            ...
            <div
              class="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 print:rounded-lg print:border-slate-300 print:p-3 print:shadow-none"
            >
              <p
              ...
              <h4
                class="mt-1 truncate text-lg font-black text-slate-900 dark:text-white"
              >
            ...
            >
              <p
                class="text-[9px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-500"
              >
                This Page
              </p>
              <h4
              ...
              >
                {{ money(pageSummary.total) }}
              </h4>
              <p
                class="mt-0.5 text-[10px] font-medium text-slate-400 dark:text-slate-500"
              >
                {{ expenses.length }} rows on this page
              </p>
            </div>
          </div>
          ...
          <!-- ==================== FILTER PANEL ==================== -->
          <div
            class="mb-6 overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 print:hidden"
          >
            <div class="space-y-5">
              <div
              ...
              <!-- Category / Sub-category / Search row -->
              <div
                class="flex flex-col gap-4 border-t border-slate-100 pt-4 dark:border-slate-800/60 lg:flex-row lg:items-end"
              >
                ...
                <div class="w-full lg:w-[220px]">
                  ...
                  <select
                    ...
                    "
                    class="w-full cursor-pointer appearance-none rounded-xl border border-slate-200 bg-slate-50/50 py-2.5 px-3 text-sm font-medium text-slate-700 outline-none transition-all focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-200 dark:focus:border-indigo-500 dark:focus:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
                  >
              ...
              <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                  <template v-if="expenses && expenses.length > 0">
                    ...
                    >
                      ...
                      <td class="px-6 py-4 print:px-3 print:py-2 max-w-xs">
                        <div
                        ...
                        >
                          {{
                            expense.remark && expense.remark !== "N/A"
                              ? expense.remark
                              : "—"
                          }}
                        </div>
                      ...
                      >
                        <span
                          class="text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md inline-flex items-center gap-1.5 border border-slate-200/60 dark:border-slate-700/50 print:bg-transparent print:border-0 print:px-0 print:py-0"
                        >
                          {{ expense.category?.name || "Uncategorized" }}
                        </span>
                        <div
                          v-if="expense.subcategory?.name"
                          class="mt-1 text-[10px] text-slate-400 dark:text-slate-500"
                        >
                      ...
                      >
                        <div
                          class="text-xs font-bold font-mono text-rose-600 dark:text-rose-400"
                        >
                  ...
                  <tr v-else>
                    <td colspan="5" class="px-6 py-12 text-center">
                      ...
                      >
                        ...
                        >
                          {{
                            errorMsg || "Try adjusting your search or filters."
                          }}
                        </p>
                      </div>
                    </td>
                ...
                >
                  <tr>
                    ...
                    >
                      Page Total ({{ expenses.length }} records)
                    </td>
                    <td
                    ...
                    <td class="print:px-3 print:py-2"></td>
                  </tr>
                </tfoot>
              </table>
            </div>
...
/* =============================================================
   ...
   NOTE: adjust these endpoints to match your actual routes.
============================================================= */
...
async function fetchCategories() {
  try {
    const res = await api.get("/expenses"); // adjust endpoint if needed
...
const filteredSubcategories = computed(() => {
  ...
  return subCategories.value.filter(
    (sub) => Number(sub.category_id) === Number(categoryId.value),
  );
});
...
async function fetchExpenses(page = 1) {
  try {
    ...
    summary.value = {
      ...
      average_expense: Number(data?.summary?.average_expense ?? 0),
    };

    categoryWise.value = data?.category_wise ?? [];

    pagination.value = {
      page: data?.expenses?.current_page ?? 1,
      lastPage: data?.expenses?.last_page ?? 1,
      total: data?.expenses?.total ?? 0,
      perPage: data?.expenses?.per_page ?? 20,
      from: data?.expenses?.from ?? 0,
      to: data?.expenses?.to ?? 0,
    };
  } catch (err) {
    console.log(err);

    errorMsg.value = err?.response?.data?.message || "Failed to fetch expenses";
...
/* =============================================================
   Page-scoped summary (sum of rows currently loaded)
============================================================= */
